...
<center>![""](./assets/welcome.webp)</center>
...
cor(azdata$tot_cases, azdata$tot_death)
cor.test(azdata$tot_cases, azdata$tot_death)
```
```{r}
kydata <- new_data[new_data$state %in% c("KY"), ]
cor(kydata$tot_cases, kydata$tot_death)
cor.test(kydata$tot_cases, kydata$tot_death)
```

```{r}
nddata <- new_data[new_data$state %in% c("ND"), ]
cor(nddata$tot_cases, nddata$tot_death)
cor.test(nddata$tot_cases, nddata$tot_death)
```
...
df <- data.frame(x=x1, yy1 = y1, yy2=y2, yy3 = y3)
df
```
```{r}
...
plt <- ggplot(df, aes(x)) +  
    geom_line(aes(y = yy1), color = "black") +
     geom_line(aes(y = yy2), color = "red") +
    geom_line(aes(y = yy3), color = "green")+scale_x_date(date_breaks = "2 week",date_labels = "%Y-%m")
plt
```
...
df <- data.frame(x=d1, yy1 = y1, yy2=y2, yy3 = y3)
df
```
```{r fig.height=5, fig.width=10}
plt <- ggplot(df, aes(x)) +  
    geom_line(aes(y = yy1), color = "black") +
     geom_line(aes(y = yy2), color = "red") +
    geom_line(aes(y = yy3), color = "green")
plt
```
...
```{r}
regressor <- lm(consent_cases ~ state + tot_cases + new_case + tot_death + new_death, data=train_mlr)
summary(regressor)
```
...
```{r}
mean_squared_error <- mean(( y_pred - test_mlr$consent_cases)^2)
mean_absolute_error <- mean(abs(y_pred-test_mlr$consent_cases))
...
data.frame(
  Prediction = y_pred,
  "Actucal Values" = test_mlr$consent_cases
)
```
...
```{r}
# ROC-AUC Curve
ROCPred <- prediction(y_pred, test_mlr$consent_cases) 
...
```{r}
logistic_model <- glm(consent_cases ~ state + tot_cases + new_case + tot_death + new_death, data=train_mlr, 
                      family = "binomial")
...
```{r}
# ROC-AUC Curve
ROCPred <- prediction(predict_reg, df$Actucal.Values)
ROCPer <- performance(ROCPred, measure = "tpr", 
                             x.measure = "fpr")
auc <- performance(ROCPred, measure = "auc")
auc <- auc@y.values[[1]]
...
<center>![""](./assets/ThankYou.jpg){width=80%} </center>
